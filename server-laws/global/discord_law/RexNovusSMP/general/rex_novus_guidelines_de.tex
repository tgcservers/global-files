...
\begin{enumerate}[(1)]
	\item Hat eine Person nachweislich Verbrechen gegen die geschriebene Verfassung einer Nation begangen, so kann diese bei der Rechtsabteilung des Servers einen Antrag
	auf einen Haftbefehl stellen.
	\item Der Rechtsabteilung muss hierbei nachvollziehbar dargelegt werden, dass und inwiefern ein Verbrechen durch die jeweilige Person begangen wurde.
	\item Sieht die Rechtsabteilung einen Haftbefehl zu genüge gerechtfertigt, so muss überprüft werden, wo sich die fragliche Person befand, als der Haftbefehl beantragt wurde.
	...
	\item Befand sich die Person im Inland oder wurde der Auslieferung zugestimmt, so wird per Zufallsverfahren entschieden, ob die Haft erfolgreich verlief.
	\item Wurde der Auslieferung nicht zugestimmt, so steht der Haftbefehl aus, bis der Auslieferung zugestimmt wurde. Verlief die Haft nicht erfolgreich, so verliert der Haftbefehl seine Wirkung.
	\item Sofern die Haft umstandslos erfolgte, wird ein Haftzwang erwirkt. Hierbei handelt es sich um die serverrechtliche Aufforderung, sich dem Gerichtsverfahren und gegebenenfalls der Strafe, zu stellen.
	\item Geht eine Person der Aufforderung nicht nach, hat dies serverrechtliche Konsequenzen und die Haft wird mittels Operatorbefehlen erzwungen.
\end{enumerate}
